module.exports.run = async (client, message, args) => {
    ...
    return message.channel.send(`Désolé, mais vous n'avez pas la permission requise pour executer cette commande.`);
        ...
        .addField(`${emojis.backup} Configuration de L\'**autorole**", 'Veuillez **cliquer sur la réaction** ci-contre pour __configurer__ l\'**autorole**`)
     ...
     collector.on('collect', async r => { 
                ...
                if(r.emoji.name === "1️⃣") {
                    message.channel.send(`${emojis.backup} *Veuillez __entrée__ l'__ID__ du __rôle__ à configurer.*`).then(mp => {
                        ...
                        db.set("autorole_role_" + message.guild.id, role.id)
                                     ...
                                     message.channel.send(`${emojis.settings} **Vous venez de __changer__ le rôle en \`${role.name}\`**`)
                                     m.edit({ embed: { title: `${emojis.backup}  Configuration de L'**autorole**`, description: 'Veuillez **cliquer sur la réaction** ci-contre pour __configurer__ l\'**autorole**',  } });         
                                    });
