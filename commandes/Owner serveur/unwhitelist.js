module.exports.run = async (client, message, args) => {
    ...
    if(message.author === message.guild.owner.user) {
        ...
        if(db.get("whitelist_" + message.guild.id + "_" + member.id) !== true) return message.channel.send(`**${emojis.non} Cette personne n'est pas whitelist.**`)

        let embed = new MessageEmbed()
        .setAuthor(message.author.username, message.author.displayAvatarURL({ dynamic: true }))
        .setTitle(`${emojis.oui} Succès :`)
        .setDescription(`**${emojis.fleched} ${member.user.tag} a bien été unwhitelist.**`)
        .setColor("303136")
        .setFooter(client.user.username)

        db.set("whitelist_" + message.guild.id + "_" + member.id, false)
